    ...
    <mesh renderOrder={-1}>
      ...
      <shaderMaterial
        ...
        fragmentShader={`
          ...
          varying vec3 vDir;
          void main() {
            float h = smoothstep(-0.18, 0.62, vDir.y);
...
export default function SkyRig({ look }: Props) {
  ...
  const sunY = Math.max(look.sunElevation, -0.15) * sunDistance;
  const sunX = Math.cos(look.sunAzimuth) * sunDistance * 0.85;
  const sunZ = Math.sin(look.sunAzimuth) * sunDistance * 0.85;
