    ...
    <mesh renderOrder={-1}>
      ...
      <shaderMaterial
        ...
        fragmentShader={`
          ...
          varying vec3 vDir;
          void main() {
            float h = smoothstep(-0.14, 0.56, vDir.y);
...
export default function SkyRig({ look }: Props) {
  ...
  const sunDistance = 56;
  const sunY = Math.max(look.sunElevation, 0.04) * sunDistance;
  const sunX = Math.cos(look.sunAzimuth) * sunDistance * 0.92;
  const sunZ = Math.sin(look.sunAzimuth) * sunDistance * 0.92;
  ...
  return (
    <>
      ...
      <Environment files="/hdri/harbour-sunset.hdr" background={false} environmentIntensity={look.envGain} />
