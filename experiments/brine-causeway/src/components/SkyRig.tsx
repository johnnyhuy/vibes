    ...
    <mesh renderOrder={-1}>
      ...
      <shaderMaterial
        ...
        fragmentShader={`
          ...
          varying vec3 vDir;
          void main() {
            float h = smoothstep(-0.12, 0.55, vDir.y);
...
export default function SkyRig({ look }: Props) {
  ...
  const sunDistance = 64;
  const sunY = Math.max(look.sunElevation, 0.04) * sunDistance;
  const sunX = Math.cos(look.sunAzimuth) * sunDistance * 0.95;
  const sunZ = Math.sin(look.sunAzimuth) * sunDistance * 0.95;
  ...
  return (
    <>
      ...
      <Environment files="/hdri/coast-fire.hdr" background={false} environmentIntensity={look.envGain} />
