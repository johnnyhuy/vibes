...
- **Three finishes** — Studio, Midnight, Citrus. Hotspots on the driver, cushion, yoke, and the mute rocker.
- **Mute-default audio** — Web Audio oscillators for a click / whoosh. Browsers block autoplay; the toggle starts **Muted**.
- **Cup-rocker mute click** — I added a small PBR mute rocker on the licensed cup. HUD Mute and a click on that rocker share one action: the paddle snaps, the yoke screw ratchets, and a short ember / emissive flash reads at the contact. `prefers-reduced-motion` snaps the pose and skips the spray. No new third-party assets.
...
Headed local loop: `docs/previews/audio-gadget-spin.gif`. Auto-spin on the licensed cups, mute left on. The cup rocker is the mechanical detail on this pass. The old three-frame clip was basically a still.
