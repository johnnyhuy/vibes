...
Pending apps (scroll-product, semicircle) need a commit that **touches their Root Directory** or a **dashboard Redeploy** that bypasses ignore, after quota ~2026-09-08 12:55 UTC. steam-atlas and japanese-tower already have LIVE production. A sibling-folder merge will skip the pending ones again.
...
3. **Skip** `vibes-japanese-tower` — LIVE Ridge Pagoda at [vibes-japanese-tower.vercel.app](https://vibes-japanese-tower.vercel.app) (`dpl_Gap9BAXosU4jMEsQsR8c2nKyEPjg`). Redeploy only if visual QA fails.
4. `vibes-blender-semicircle` — production still `25587f54` (cropped mega-arc / thin vertical ring). One `main` redeploy for bbox framing.
5. `vibes-scroll-product` — first READY production from `main` (Root already `experiments/scroll-product-showcase`).
6. Glass is already live. Do **not** create `vibes-ballance-roll`, a courtyard project, or `vibes-audio-gadget-spin` until quota is healthy.
...
This pass adds Lumen Cuff locally. **No redeploy.** japanese-tower is **LIVE** Ridge Pagoda (`dpl_Gap9BAXosU4jMEsQsR8c2nKyEPjg`). Post-quota jobs are semicircle then scroll. Do not create audio-gadget / ballance-roll / courtyard projects. Skip explode, steam, and tower unless visual QA fails.
