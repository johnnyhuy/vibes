# ADR-0024: A Soft Outdoor Memory Walk, Invented Residential Lane

**Date**: 2026-09-08  
**Status**: Accepted  
**Context**: [vibes](../../) monorepo  
**Related**: [fairday-walk.md](../reverse-engineering/fairday-walk.md), [fairday-walk](../../experiments/fairday-walk/), [ADR-0005](./0005-scroll-driven-product-hero.md), [ADR-0020](./0020-alba-forum-educational-landmark-scroll.md), [ADR-0004](./0004-procedural-geometry-over-assets.md), [ADR-0023](./0023-keel-hex-assembly.md)

## Context

@anyumeng28 posted that GPT-6 Astra’s modelling only feels real in use: Three.js turned pictures into a 3D version, about half a Plus weekly quota, Blender refinement mentioned, old photos into a 3D webpage you can walk again. The live they linked reads as a soft outdoor memory diorama you can orbit. I wanted the *lesson* — photo→3D feel, a breezy folio you walk then inspect — without scraping their site, cloning their title, or inventing another chalk forum.

Alba Forum (ADR-0020) already maps window scroll onto landmark poses. Keel Hex took ADR-0023. Chinese-courtyard already orbits a siheyuan. Hobby Vercel quota is still 0 until ~2026-09-08 20:39 UTC, so the experiment stays local. **No Vercel project.**
...
- Reusing 晴天 / Qing Tian / anyumeng / 风里的记忆 NO. 001 nouns
- Extending Alba Forum with a white laundry arch
- Another dusk harbour / coast drive / hex bench

**Why**: The public hook is “walk a memory, then orbit the place in front of you.” A chalk avenue and a pagoda lift already exist. The hypothesis is that pose lerp + Orbit is enough, on a *residential* folio.
...
**Why**: Clean-room. The pattern is “soft outdoor memory diorama.” The lane has to be one I can stand behind.

### 4. Orbit is a mode; Recast returns the authored pose

**Chosen**: `OrbitControls` enabled only in Orbit. Canvas `pointer-events` flip on. Wheel is captured so zoom does not steal the page. Recast (button, `R`, or double-click) restores the stop pose. Folio / Esc damps back to the scroll framing.

**Rejected**: Always-on orbit (that is japanese-tower). A separate route per place. Copying their bottom-pill Chinese.
...
**Rejected**: Creating a project while quota is 0 / while the Hobby 25-link cap is already blocking `vibes-keel-hex`.
...
2. Fair-day residential read is distinct from chalk / kiln / harbour / hex siblings
3. Orbit is a mode, not a second camera stack
