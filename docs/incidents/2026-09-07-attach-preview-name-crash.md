...
**Status**: Fixed (RCA retained; CarModel patches from PR #11 superseded by #12)  
...
**Code landing**: PR #12 (`0ec7b5d`) rewrote explode-assembly with `polishMaterial()` (`??` defaults + `'metalness' in mat` guards), attach/layout, and try/catch around material processing. Do not replay the pre-gallery `CarModel.tsx` hunks from PR #11.
...
- **PR #12** (commit 0ec7b5d): Ordered gallery rewrite — current null-safety + attach/layout on `main`
- **PR #11**: Closed as superseded; CarModel patches were against the pre-#12 file
